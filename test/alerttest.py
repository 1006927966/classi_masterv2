...
if __name__ == '__main__':
    ...
    def test(epoch,  day, version):
        ...
        img_dir = '/defaultShare/share/wujl/83/online_data/cropdata/{}'.format(day)
        txt_dir = '/defaultShare/share/wujl/83/online_data/'
        os.makedirs(txt_dir, exist_ok=True)
        print(img_dir)
        save_txt = os.path.join(txt_dir, '{}_{}_{}.txt'.format(day, epoch, version))
        print(save_txt)
        ...
        data_loader = DataLoader(data_set, batch_size=64, num_workers=16, shuffle=False)
        score_list = list()
        label_list = list()
        for index, img in enumerate(data_loader):
            ...
            print('{}/{}'.format((index + 1) *len(list(batch_score['score'][1])), len(name_list)))
