...
class TestData(data.Dataset):
    def __init__(self, train=False, transform=None):
        ...
        self.img_list = list()
        name_list = os.listdir(os.path.join(self.root, '0'))
        for name in name_list:
            self.img_list.append(os.path.join(self.root, '0', name))
        name_list = os.listdir(os.path.join(self.root, '1'))
        for name in name_list:
            self.img_list.append(os.path.join(self.root, '1', name))
